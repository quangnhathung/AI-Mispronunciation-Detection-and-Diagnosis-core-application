...
def train_model_v3(epochs=25):
    ...
    # 3. TỐI ƯU HÓA PHÂN TẦNG (Cập nhật cho V3)
    optimizer = torch.optim.AdamW([
        {'params': model.wav2vec2.parameters(), 'lr': 1e-5},
        {'params': model.phoneme_embedding.parameters(), 'lr': 1e-4},
        {'params': model.phoneme_rnn.parameters(), 'lr': 1e-4}, # <--- DÒNG MỚI THÊM VÀO
    ...
    for epoch in range(num_epochs):
        ...
        if f1_score > best_f1_error:
            ...
            save_path = os.path.join(save_dir, "best_mdd_model_v3.pt")
